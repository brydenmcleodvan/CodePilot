...
def fetch_all_patients():
    ...
    with engine.connect() as conn:
        ...
        patients = []
        for row in result:
            # Convert row to dictionary explicitly with indices
            patients.append({
                'id': row[0],
                'display_name': row[1],
                'age_at_record': row[2],
...
def fetch_patient_data(patient_id):
    ...
    with engine.connect() as conn:
        ...
        patient_row = patient_result.fetchone() if patient_result.rowcount > 0 else None
        personal_info = {}
        if patient_row:
            personal_info = {
                'id': patient_row[0],
                'display_name': patient_row[1],
                'age_at_record': patient_row[2],
                'physical_description': patient_row[3],
                'record_date': patient_row[4]
            }
        ...
        diagnostic_tests_rows = []
        for row in diagnostic_tests_result:
            diagnostic_tests_rows.append({
                'test_type': row[0],
                'test_results': row[1]
            })
        ...
        if social_health_row:
            social_health_dict = {
                'impact_on_daily_life': social_health_row[0],
                'support_network': social_health_row[1],
                'self_management': social_health_row[2]
            }
    ...
    if medical_history_row:
        medical_history_dict = {
            'neonatal_brain_injury': medical_history_row[0],
            'seizure_type': medical_history_row[1],
            'seizure_frequency': medical_history_row[2],
            'migraines': medical_history_row[3],
            'headaches': medical_history_row[4]
        }
    ...
    # Add social health
    if social_health_row:
        patient_data["social_health"] = social_health_dict
...
def add_new_patient(patient_data):
    ...
    with engine.begin() as conn:
        # Insert patient
        patient_query = """
        INSERT INTO patients (display_name, age_at_record, physical_description, record_date, created_at)
        VALUES (:display_name, :age_at_record, :physical_description, :record_date, CURRENT_TIMESTAMP)
        RETURNING id
        """
...
def import_json_to_database(json_file_path):
    """Import a neural profile JSON file into the database"""
    try:
        # Read the JSON file
        with open(json_file_path, 'r') as file:
            json_data = json.load(file)
        
        # Extract the profile data and add it to the database
        profile_data = json_data.get('profile', {})
        if not profile_data:
            return None, "Invalid JSON format: 'profile' key missing"
            
        # Add the patient to the database
        patient_id = add_new_patient(profile_data)
        return patient_id, "Successfully imported patient data"
    except Exception as e:
        return None, f"Error importing JSON: {str(e)}"